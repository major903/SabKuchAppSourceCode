...
public class MyClassifiedsAdapter extends RecyclerView.Adapter<MyClassifiedsAdapter.ViewHolder> {
    private Context context;
    private ArrayList<Classified> classifieds;
    private OnListViewItemClickListener listener;

    MyClassifiedsAdapter(Context context, ArrayList<Classified> classifieds, OnListViewItemClickListener listener) {
    ...
    @NonNull
    @Override
    public MyClassifiedsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int i) {
        LayoutInflater layoutInflater = LayoutInflater.from(context);
        View view = layoutInflater.inflate(R.layout.fragment_my_classified_details_list_item, parent, false);
        return new MyClassifiedsAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MyClassifiedsAdapter.ViewHolder holder, int position) {

        Classified classified = classifieds.get(position);

        UiUtil.setTextView(holder.tvUserName, DateTimeUtils.getFormattedDate(classified.getUpdatedAt(),
        ...
        holder.btEdit.setOnClickListener(v -> {
            if (listener != null)
                listener.onItemClick(classified, position, holder.itemView, ListItemClickAction.EDIT);
        });

        holder.btDelete.setOnClickListener(v -> {
            if (listener != null)
                listener.onItemClick(classified, position, holder.itemView, ListItemClickAction.DELETE);
        });
